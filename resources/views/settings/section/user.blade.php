    ...
    <div class="flex flex-col md:flex-row gap-2 items-start md:space-x-6 p-4 bg-gray-100 rounded-md shadow">
        ...
        <div class="w-full md:w-3/4">
            <div class="mb-4 px-2 py-2">
                ...
                <select id="disable_create_user_by_command" name="disable_create_user_by_command"
                    ...
                    <option value="1" {{ settingItem('disable_create_user_by_command') == 1 ? 'selected' : '' }}>
                        {{ __('Yes') }}
                    </option>
                    <option value="0" {{ settingItem('disable_create_user_by_command') == 0 ? 'selected' : '' }}>
                        {{ __('No') }}
                    </option>
                </select>
            ...
            <div class="mb-4 px-2 py-2">
                ...
                <input id="destroy_user_after" type="number" name="destroy_user_after"
                    class="block w-full px-2 py-2 rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
                    placeholder="{{ __('Enter the number of days after which users will be destroyed') }}"
                    value="{{ settingItem('destroy_user_after') }}">
                <small
