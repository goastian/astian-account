<?php
namespace App\Http\Controllers\Web\OAuth\OpenId;
...
final class DiscoveryController extends \OpenIDConnect\Laravel\DiscoveryController
{
    public function __invoke(Request $request, LaravelCurrentRequestService $currentRequestService)
    {
        ...
        if (Route::has('gateway.user.info')) {
            $response['userinfo_endpoint'] = route('gateway.user.info');
        }
        ...
        if (Route::has('logout')) {
            $response['end_session_endpoint'] = route('logout');
        }
